...
try{
    ...
    if( $id ){
        ...
        ?>
        <h2>Wins for <?= $game->name ?></h2>
        <?
        $result = $api->call( $game->links->game_prize_wins, 'GET', array(
            'limit' => 10000
        ));
        ?>
...
function wins_button( $game )
{
    if( @$game->links->game_prize_wins ){
        ?>
    <form action="?" method="POST" style="margin:0">
        <input type="hidden" name="id" value="<?= $game->id ?>" />
        <button type="submit" class="btn btn-mini">View Wins</button>
    </form>
        <?php
    }
    else {
